...
def create_perfetto_trace(
    ...
    # Find all unique PIDs from nn.Module events
    nn_module_pids = set()
    for event in nn_module_events:
        pid = event.get("pid")
        if pid is not None:
            nn_module_pids.add(pid)
    
    # Create a mapping for new PIDs (add suffix for nn.Module view)
    # We'll create a new process for each original PID
    pid_mapping = {}
    for pid in nn_module_pids:
        # Create new PID name for nn.Module view
        pid_mapping[pid] = f"{pid} [Model Structure]"
    
    # Clone nn.Module events with new PID names
    cloned_nn_module_events = []
    for event in nn_module_events:
        cloned_event = event.copy()
        original_pid = event.get("pid")
        if original_pid in pid_mapping:
            cloned_event["pid"] = pid_mapping[original_pid]
        cloned_nn_module_events.append(cloned_event)
    
    # Combine all events: original events + kernel events + cloned nn.Module events
    # Note: original events already include the original nn.Module events
    combined_events = all_events + cloned_nn_module_events
    ...
    # Add metadata events for the new Model Structure processes
    for original_pid, new_pid in pid_mapping.items():
        # Add process name metadata
        perfetto_trace["traceEvents"].append({
            "name": "process_name",
            "ph": "M",
            "pid": new_pid,
            "args": {
                "name": "Model Structure (nn.Module)"
            }
        })
        
        # Add process sort index (place it after the original process)
        # Find original process sort index
        original_sort_index = 0
        for event in all_events:
            if (event.get("ph") == "M" and 
                event.get("name") == "process_sort_index" and 
                event.get("pid") == original_pid):
                original_sort_index = event.get("args", {}).get("sort_index", 0)
                break
        ...
        perfetto_trace["traceEvents"].append({
            ...
            "pid": new_pid,
            "args": {
                "sort_index": original_sort_index + 1000  # Place after original process
            }
        })
...
def main():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Path to output JSON file (default: <input>_kernel_module.json.gz)"
    )
    parser.add_argument(
        "--no-compress",
        action="store_true",
        help="Do not compress output file (default is to compress with gzip)"
    )
    parser.add_argument(
        "--no-analysis",
        action="store_true",
    ...
    if args.output is None:
        ...
        # Default to compressed format unless --no-compress is specified
        if args.no_compress:
            args.output = f"{base_name}{suffix}.json"
        else:
            args.output = f"{base_name}{suffix}.json.gz"
