...
class ProfileMerger:
    ...
    def merge_chrome_traces(
        self, file_names: List[str], suffix: str = "trace.json.gz"
    ) -> str:
        ...
        """
        trace_files = self._discover_trace_files(file_names, suffix)
        if not trace_files:
    ...
    def _discover_trace_files(
        ...
        patterns = [f"{self.profile_id}*.{suffix}"] if suffix else file_names

        trace_files = []
        for pattern in patterns:
            search_pattern = os.path.join(self.output_dir, pattern)
            trace_files.extend(glob.glob(search_pattern))
        assert trace_files, "No trace files found, No files nedd to be merged."
        trace_files = [
            f
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Merge Chrome trace files from multiple ranks."
    )
    parser.add_argument(
        "--output_dir",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        required=True,
        help="Profile ID to identify trace files.",
    )
    parser.add_argument(
        "--suffix",
        type=str,
        default="trace.json.gz",
        help="Suffix of trace files to merge (default: trace.json.gz).",
    )
    ...
    parser.add_argument(
        ...
        nargs="*",
        default=[],
        help="Specific trace files to merge (overrides suffix if provided).",
    )

    parsed_args = parser.parse_args()

    merger = ProfileMerger(parsed_args.output_dir, parsed_args.profile_id)
    merged_path = merger.merge_chrome_traces(
        file_names=parsed_args.trace_files, suffix=parsed_args.suffix
    )
    print(f"Merged trace saved to: {merged_path}")
